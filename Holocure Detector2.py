...
if len(play_loc[0]) > 0:
    ...
    if len(question_loc[0]) > 0:
        ...
        for i in range(3):
            ...
            time.sleep(0.1)
        while True:
            # Check if "game over" text is on the screen
            if pyautogui.locateOnScreen('templates/UP.png', grayscale=True, confidence=0.8) is not None:
                ...
                time.sleep(0.01)
            if pyautogui.locateOnScreen('templates/GameOver.PNG', grayscale=True, confidence=0.6) is not None:
                print("Game over!")
                break
